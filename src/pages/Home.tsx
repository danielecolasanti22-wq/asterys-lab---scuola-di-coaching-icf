...
const Testimonianze = () => {
  ...
  useEffect(() => {
    const urls = videoTestimonials
      .map((t) => t.video?.vimeoEmbedUrl)
    ...
    Promise.all(
      urls.map(async (url) => {
        ...
        return data.thumbnail_url ? { url, thumbnail: data.thumbnail_url.replace(/_\d+x\d+\./, '_1280x720.') } : null;
      })
    )
      .then((results) => {
        if (cancelled) return;
        ...
        results.forEach((item) => {
          if (!item) return;
          next[item.url] = item.thumbnail;
        });
        setVimeoThumbnails(next);
      })
      .catch(() => undefined);
          ...
          <div className="relative overflow-hidden">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                >
                  <img
                    src={slides[activeSlide].video.video?.vimeoEmbedUrl ? (vimeoThumbnails[slides[activeSlide].video.video.vimeoEmbedUrl] ?? slides[activeSlide].video.video?.poster) : slides[activeSlide].video.video?.poster}
