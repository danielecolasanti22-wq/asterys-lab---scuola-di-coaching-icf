...
export default function Courses() {
  ...
  return (
    <div className="pt-32 pb-20">
      <section className="bg-brand-navy text-white py-20 mb-12 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-1/3 h-full bg-brand-accent/5 skew-x-12 -mr-32"></div>
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex items-center gap-2 mb-6 text-brand-accent uppercase text-xs font-black tracking-widest">
            <span className="w-8 h-px bg-brand-accent"></span>
            Asterys Lab Academy
          </div>
          <h1 className="font-display font-bold text-5xl lg:text-7xl mb-6 tracking-tight">Tutti i percorsi.</h1>
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all ${
                activeCategory === cat 
                  ...
                  : 'bg-brand-blue-soft/50 text-brand-navy/60 hover:bg-brand-blue-soft hover:text-brand-navy'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredCourses.map((c, i) => (
            <motion.div 
              ...
              key={c.id} 
              className="bg-white rounded-3xl overflow-hidden flex flex-col shadow-soft hover:shadow-xl transition-all border border-gray-100 group"
            >
              <div className="relative aspect-[16/10] overflow-hidden">
                <img 
                  ...
                  alt={c.title} 
                />
                <div className="absolute top-4 left-4 inline-block px-3 py-1 bg-white/95 backdrop-blur-sm rounded-full text-[10px] font-black tracking-widest text-brand-navy">
                  {c.type}
                </div>
              </div>
              <div className="p-8 flex flex-col flex-grow">
                <div className="flex gap-2 mb-4 flex-wrap">
                  {c.tags.slice(0, 2).map(t => (
                    <span key={t} className="text-[10px] bg-brand-blue-soft px-2 py-0.5 rounded font-black text-brand-navy/40 uppercase tracking-wider">{t}</span>
                  ))}
                </div>
                <h3 className="font-display font-bold text-xl mb-4 leading-tight group-hover:text-brand-accent transition-colors">{c.title}</h3>
                ...
                <Link to={`/corsi/${c.id}`} className="btn-primary w-full py-4 text-sm flex justify-center items-center gap-2">
      ...
      <section className="mt-32 section-padding bg-brand-blue-soft/30">
        <div className="max-w-7xl mx-auto bg-brand-navy rounded-[3rem] p-12 lg:p-20 text-white relative overflow-hidden shadow-2xl">
